...
```{r}
rfb3 <- train(as.factor(change2) ~ .  - landuse - change,
            data = train2, ## use the same data here as we used in spatialBlock()?
            method = "ranger",
            trControl = umFolds)
plot(rfb3)
predb3 <- predict(rfb3, newdata=test2, typeprobs="prob")
conb <- confusionMatrix(predb3, as.factor(test2$change2))
table(test2$change2, predb3)
```
H-P:Error in ROC
```{r roc_random_forest_buffer3, message=TRUE}
rocCurve <- pROC::roc(factor(test2$change,levels=c("change","no change")),
                      predb3[,"change"])
...
auc(rocCurve)
```
H-P:Error: `data` and `reference` should be factors with the same levels.
```{r}
conb1 <- confusionMatrix(predb1, as.factor(b$change2))
conb2=confusionMatrix(
  factor(predb1, levels = 1:3),
  factor(b$change2, levels = 1:3)
)
```
may be below method is not coorect for cross validation
