...
train2 <- dat_upsample[train,]
test2 <- dat_upsample[test, ]
...
rf_b <- train(as.factor(change2) ~ .  - landuse - change,
            ...
            trControl = umFolds)
plot(rf_b)
pred <- predict(rf_b, newdata=test2, typeprobs="prob")
conb <- confusionMatrix(pred, as.factor(test2$change2))
table(test2$change2, pred)
