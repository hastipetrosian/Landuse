...
```{r fitted random_forest, cache=TRUE}
## use train data
...
```
check overfiiting:if training error is lower than our approximation of generalization error via the test error, we have over fitting
```{r acuuracy of train data}
...
```{r not fitted random forest}
if (file.exists("rf.RData")) {
    load("rf.RData")
} else {
    rf <- randomForest(formula= factor(change2) ~ . - x - y ,
                       data = sample_train, n.trees=250,interaction.depth=7, do.trace=1,
...
```{r}
ggplot(  ) + 
  geom_point( aes(x = sample_test2$landuse, y = ?, color = 'red', alpha = 0.5) ) + 
  geom_point( aes(x =  sample_test2$landuse , y = rf_pred3, color = 'blue',  alpha = 0.5)) + 
