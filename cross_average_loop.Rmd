...
class(PA_data2) ## the class is not sf
##why I chose above method
##change list to data frame
dat_upsampleall2=as.data.frame(dat_upsampleall)
##st_as_sf_all
for (i in seq_along(dat_upsampleall2)) 
    PA_data2[[i]] <-
    st_as_sf(x=dat_upsampleall2[[i]]~ .,  
    coords = c("x", "y"), crs = crs(Classified1))
```

```{r Loop_years_and_blocks}
rflist2 <- list()
pts <- seq(nrow(dat_upsampleall2))
for (i in seq(nrow(corners))) { ## set i = 1, 2, 3, ... number of squares (9)
     ## i <- 1
     cat(i,"\n")
     test2 <- pts[dat_upsampleall2$x > corners[i,"x1"] &
                 dat_upsampleall2$x < corners[i,"x2"] &
                 dat_upsampleall2$y < corners[i,"y1"] &
                 dat_upsampleall2$y > corners[i,"y2"]]
     ## select train values as everything else
     train2 <- setdiff(pts,test2)
     numFolds <- trainControl(method = "cv", number = 10)
      ## ???
     ## i is 9 but for PA_data2, i should be 1 to 6
     for (i in seq_along(PA_data2)) {
     train2all <- PA_data2[[i]][train2,]
     test2all <- PA_data2[[i]][test2, ]
     train2all$geometry <- NULL
     test2all$geometry <- NULL
     rflist2[[i]] <- randomForest(formula= factor(change2) ~ . - landuse - change,
                         data = train2all,
                         n.trees=400,interaction.depth=7,
                         ## do.trace=1,
                         type="classification", proximity=TRUE, trcontrol=numFolds, keep.inbag = TRUE)
     ## do the predictions, etc 
     predbc22 <- predict(rflist2[[i]], newdata=test2all, typeprobs="prob", predict.all=TRUE)
