...
datall <- map(date,~ dat)
dat_gainall <- map(datall,~ dat_gain)

dat_upsample <- ROSE::ovun.sample(change ~ ., data = dat_gainall, method = "both", N=1500)$data
dat_upsampleall <- map(dat_gainall, dat_upsample)
```
